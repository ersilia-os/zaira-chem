...
class Reg_EarlyStoppingAndPerformance(tf.keras.callbacks.Callback):
    def __init__(
        self, train_data, valid_data, MASK=-1, patience=5, criteria="val_loss"
    ):
        super(Reg_EarlyStoppingAndPerformance, self).__init__()

        assert criteria in ["val_loss", "val_r2"], "not support %s ! only %s" % (
            criteria,
            ["val_loss", "val_r2"],
        )
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        print(
            "\repoch: %s, loss: %s - val_loss: %s; rmse: %s - rmse_val: %s;  r2: %s - r2_val: %s"
            % (eph, loss, val_loss, rmse, rmse_val, r2_mean, r2_mean_val),
            end=100 * " " + "\n",
        )
...
class CLA_EarlyStoppingAndPerformance(tf.keras.callbacks.Callback):
    ...
    def roc_auc(self, y_true, y_pred):
        ...
        for i in range(N_classes):
            ...
            try:
                if self.metric == "ROC":
                    auc = roc_auc_score(
                        y_true_one_class[mask], y_pred_one_class[mask]
                    )  # ROC_AUC
                    ...
                    auc = accuracy_score(
                        y_true_one_class[mask], np.round(y_pred_one_class[mask])
                    )  # ACC
            except:
                auc = np.nan
            aucs.append(auc)
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        if self.metric == "ACC":
            print(
                "\repoch: %s, loss: %s - val_loss: %s; acc: %s - val_acc: %s"
                % (eph, loss, val_loss, auc, auc_val),
                end=100 * " " + "\n",
            )

        else:
            print(
                "\repoch: %s, loss: %s - val_loss: %s; auc: %s - val_auc: %s"
                % (eph, loss, val_loss, auc, auc_val),
                end=100 * " " + "\n",
            )
