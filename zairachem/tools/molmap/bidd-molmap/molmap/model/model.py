...
class RegressionEstimator(BaseEstimator, RegressorMixin):
    ...
    def __init__(
        ...
        assert y_scale in [
            ...
            "minmax",
            "standard",
        ], "scale_y should be None, or 'minmax', or 'standard'!"
        if y_scale == None:
            y_scaler = None
        elif y_scale == "minmax":
    ...
    def fit(self, X, y, X_valid=None, y_valid=None):
        ...
        if self.fmap_shape2 is None:
            if X.ndim != 4:
                raise ValueError(
                    "Found array X with dim %d. %s expected == 4." % (X.ndim, self.name)
                )
            w, h, c = X.shape[1:]
            w_, h_, c_ = self.fmap_shape1
            assert (w == w_) & (h == h_) & (c == c_), (
                "Input shape of X is not matched the defined fmap_shape. expected == %s"
                % self.fmap_shape1
            )
            ...
            w1, h1, c1 = X1.shape[1:]
            w2, h2, c2 = X2.shape[1:]
            assert (w1 == w1_) & (h1 == h1_) & (c1 == c1_), (
                "Input shape of X is not matched the defined fmap_shape. expected == %s"
                % self.fmap_shape1
            )
            assert (w2 == w2_) & (h2 == h2_) & (c2 == c2_), (
                "Input shape of X is not matched the defined fmap_shape. expected == %s"
                % self.fmap_shape2
            )
        ...
        performance = Reg_EarlyStoppingAndPerformance(
            (X, y),
            (X_valid, y_valid),
...
class MultiClassEstimator(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, X, y, X_valid=None, y_valid=None):
        ...
        performance = CLA_EarlyStoppingAndPerformance(
            (X, y),
            (X_valid, y_valid),
            patience=self.patience,
            criteria=self.monitor,
