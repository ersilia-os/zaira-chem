...
class GlobAASeqMolMap(BaseMap):
    def __init__(
        ...
        print_info(
            "Total Sequences to calculate: %s, feature parameters: %s"
            % (self.seq_num, self.feature_para_dict)
        )
        print_info(
            "Total features to calculate: %s, details: %s" % (feat_num, bitsinfodict)
        )
        ...
        ## init AggMap
        BaseMap.__init__(self, dfx, metric=metric, info_distance=info_distance)
        self.dfx = dfx
    ...
    def transform(self, seq, scale=True, scale_method="minmax"):
        ...
        fmap = self._transform(arr_1d, scale=scale, scale_method=scale_method)
        return fmap

    def batch_transform(self, seq_list, scale=True, scale_method="minmax", n_jobs=4):
        ...
        fmap = self._batch_transform(
            array_2d, scale=scale, scale_method=scale_method, n_jobs=n_jobs
        )
...
class LocalAASeqMolMap:
    ...
    def __init__(self):
        ...
        index1_subgroup = (
            self.allindex.meta.set_index("accession_number")
            .loc[self.index1]["subgroup"]
            .to_dict()
        )
        self.df_index1["subgroup"] = self.df_index1.accession_number.map(
            index1_subgroup
        )
    ...
    def get_matrices_mutation(self, index_key="GONG920101"):
        ...
        ts = (
            self.df_index2.set_index("accession_number")
            .loc[index_key]
            .set_index("animo_acid_key")["value"]
        )
    ...
    def get_matrices_contact_potentials(self, index_key="BONM030106"):
        ...
        ts = (
            self.df_index3.set_index("accession_number")
            .loc[index_key]
            .set_index("animo_acid_key")["value"]
        )
        ts = (ts - ts.min()) / ((ts.max() - ts.min()))
