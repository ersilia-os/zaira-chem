...
class MolMap(Base):
    ...
    def transform(
        ...
        if (scale) & (self.ftype == "descriptor"):
            ...
            if scale_method == "standard":
                df = self.StandardScaler(
                    df, self.scale_info["mean"], self.scale_info["std"]
                )
            else:
    ...
    def batch_transform(self, smiles_list, scale=True, scale_method="minmax", n_jobs=4):
        ...
        res = P(
            delayed(self.transform)(smiles, scale, scale_method)
    ...
    def rearrangement(self, orignal_X, target_mp):
        ...
        """
        assert self.flist == target_mp.flist, print_error(
            "Input features list is different, can not re-arrangement, check your flist by mp.flist method"
        )
        assert len(orignal_X.shape) == 4, print_error(
            "Input X has error shape, please reshape to (samples, w, h, channels)"
        )
    ...
    def plot_scatter(self, htmlpath="./", htmlname=None, radius=3):
        ...
        df_scatter, H_scatter = vismap.plot_scatter(
            self, htmlpath=htmlpath, htmlname=htmlname, radius=radius
        )
