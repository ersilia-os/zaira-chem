...
class BaseMap(Base):
    def __init__(self, dfx, metric="correlation", info_distance=None):
        ...
        assert (
            type(dfx) == pd.core.frame.DataFrame
        ), "input dfx mush be pandas DataFrame!"
        super().__init__()
        ...
        if np.array(info_distance).any():
            assert len(info_distance) == info_distance_length, (
                "shape of info_distance must be (%s,)" % info_distance_length
            )
    ...
    def _fit(
        ...
        **kwargs
        ...
        ## embedding  into a 2d
        assert emb_method in ["tsne", "umap", "mds"], (
            "No Such Method Supported: %s" % emb_method
        )
        assert fmap_type in ["scatter", "grid"], (
            "No Such Feature Map Type Supported: %s" % fmap_type
        )
        ...
        dfd = pd.DataFrame(
            squareform(self.info_distance), index=self.alist, columns=self.alist
        )
        ...
        if feature_group_list != []:
            ...
            assert len(feature_group_list) == len(
                self.alist
            ), "the length of the input group list is not equal to length of the feature list"
            ...
            print_info(
                "applying hierarchical clustering to obtain group information ..."
            )
        ...
        for i in list(
            set(self.channel_order) - set(self.df_embedding.Channels.unique())
        ):
        ...
        if self.fmap_type == "scatter":
            ...
            print_info(
                "Applying grid feature map(assignment), this may take several minutes(1~30 min)"
            )
            self._S.fit(
                self.df_embedding,
                self.split_channels,
        ...
        if self.fmap_shape == None:
            ...
            assert (m >= p) & (
                n >= q
            ), "fmap_shape's width must >= %s, height >= %s " % (p, q)
    ...
    def _batch_transform(self, array_2d, scale=True, scale_method="minmax", n_jobs=4):
        ...
        res = P(
            delayed(self._transform)(arr_1d, scale, scale_method)
            for arr_1d in tqdm(array_2d, ascii=True)
        )
        X = np.stack(res)
    ...
    def plot_scatter(
        self, htmlpath="./", htmlname=None, radius=2, enabled_data_labels=False
    ):
        """radius: the size of the scatter, must be int"""
        df_scatter, H_scatter = vismap.plot_scatter(
            self,
            htmlpath=htmlpath,
            htmlname=htmlname,
            radius=radius,
    ...
    def plot_grid(self, htmlpath="./", htmlname=None, enabled_data_labels=False):
        ...
        df_grid, H_grid = vismap.plot_grid(
            self,
            htmlpath=htmlpath,
            htmlname=htmlname,
            enabled_data_labels=enabled_data_labels,
    ...
    def plot_tree(
        self, figsize=(16, 8), add_leaf_labels=True, leaf_font_size=18, leaf_rotation=90
    ):
