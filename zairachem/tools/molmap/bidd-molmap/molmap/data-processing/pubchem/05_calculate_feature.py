...
def calculate(npy_file_name, batch_cal_fuc, M, N, chunksize, n_jobs, dtype):
    ...
    iterator = pd.read_csv(
        datapath, index_col="index", iterator=True, chunksize=chunksize
    )

    f = np.memmap(npy_file_name, dtype=dtype, mode="w+", shape=(M, N))
...
if __name__ == "__main__":
    ...
    calculate(
        "/raid/shenwanxiang/descriptors_8506205.npy",
        D.batch_transform,
        M,
        Nd,
        chunksize,
    ...
    calculate(
        "/raid/shenwanxiang/fingerprint_8506205.npy",
        F.batch_transform,
        M,
        Nf,
        chunksize,
