...
def ecfp4(smile, bitsize=1024):
    ...
    try:
        ...
        fp = AllChem.GetMorganFingerprintAsBitVect(
            mol, radius=2, bitInfo=bitInfo, nBits=bitsize
        )
...
if __name__ == "__main__":
    ...
    fname = os.path.join(
        data_save_folder, "cid.smiles.nodupstring.%s.csv" % total_sample
    )

    f = np.memmap(
        os.path.join(data_save_folder, "%s_%s.npy" % (total_sample, bitsize)),
        dtype=np.bool,
        mode="w+",
        shape=(total_sample, bitsize),
    )

    iterator = pd.read_csv(fname, iterator=True, index_col=0, chunksize=chunksize)
