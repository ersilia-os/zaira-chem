...
if __name__ == "__main__":
    ...
    fig, (ax1, ax2) = plt.subplots(
        2, sharex=True, figsize=(12, 10), gridspec_kw={"hspace": 0.05}
    )

    alldata = df_sample["NumOnbits"]
    ax1 = alldata.hist(
        bins=100, edgecolor="black", ax=ax1, ylabelsize=14, color="lightblue"
    )

    sampled = df_sample[df_sample["sample"] == True]["NumOnbits"]
    ax2 = sampled.hist(
        bins=100, edgecolor="black", ax=ax2, ylabelsize=14, color="lightblue"
    )
