...
def clip_text(text):
    if len(text) > DEFAULT_TEXT_LENGTH:
        text = "%s %s%s... [%d chars truncated] ...%s %s" % (
            text[:DEFAULT_TEXT_LENGTH_PREFIX],
            fg("red"),
            attr("bold"),
            len(text) - DEFAULT_TEXT_LENGTH,
            attr("reset"),
            text[-DEFAULT_TEXT_LENGTH_SUFFIX:],
        )
    return text
...
def create_print_method(level):
    ...
    def func(*args, sep=" ", verbose=True):
        if verbose:
            print_method(clip_text(sep.join(map(str, args))))
...
def print_traceback():
    ...
    for frame in reversed(inspect.getouterframes(inspect.currentframe())[1:]):
        logger.info(
            '  File "%s", line %s, in %s', frame.filename, frame.lineno, frame.function
        )
