...
class MollibSampler():
    ...
    def _sample(self, smiles_list):
        ...
        cmd = "cd {0}; bash run_morty.sh {1}; cd {2}".format(self.exec_folder, self.data_file_relative, self.cwd)
        SimpleConda().run_commandlines(MOLLIB_CONDA_ENVIRONMENT, cmd)

    def _read_molecules(self):
        output_folder = os.path.join(self.exec_folder, "results", self.data_tag, "novo_molecules")
        smiles = set()
