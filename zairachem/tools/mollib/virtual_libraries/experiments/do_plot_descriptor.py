...
def do_combined_boxplot(df, desc_to_plot, epochs_to_plot, save_path, te):
    ...
    sns.boxplot(
        ...
        y="value",
        data=df,
        palette=start_palette + epoch_palette + end_palette,
        showfliers=False,
        width=0.35,
    )
    ...
    ax.set_xticklabels(
        start_xticklabels + epoch_xticklabels + end_xticklabels,
        rotation=30,
...
if __name__ == "__main__":
    ...
    dict_temp = get_dict_with_data(
        src_space_name, min_len, max_len, int(config["AUGMENTATION"]["source_space"])
    )
    ...
    for fname in sorted(os.listdir(path_des)):
        if fname.endswith(".pkl"):
            if "desc" in fname and str(temp) in fname:
                ...
                # add to dataframe
                dict_temp = {"seq_time": [seq_time] * len(values), "value": values}
                df = update_df(df, dict_temp)

    dict_temp = get_dict_with_data(
        name_data, min_len, max_len, int(config["AUGMENTATION"]["fine_tuning"])
    )
    df = update_df(df, dict_temp)
    ...
    dict_temp = get_dict_with_data(
        tgt_space_name, min_len, max_len, int(config["AUGMENTATION"]["target_space"])
    )
    df = update_df(df, dict_temp)
    ...
    epochs_to_plot = sorted(
        [x.split("/")[-1].replace(".h5", "") for x in all_models], key=int
    )
    do_combined_boxplot(df, desc_to_plot, epochs_to_plot, save_path, temp)

    end = time.time()
    if verbose:
        print(f"DESCRIPTOR PLOT DONE in {end - start:.04} seconds")
    ####################################
