...
if __name__ == "__main__":
        ...
        if filename.endswith(".pkl") and "scaf" in filename:
            ...
            if len(checked_scaf) > 0:
                ...
                if config["DATA"]["target_space"]:
                    ...
                    novo_tr_both = list(
                        unique_set - set(scaf_target) - set(scaf_source)
                    )
                ...
                if verbose:
                    print(f"scaffolds analysis for {name} done")
            else:
    ...
    if verbose:
        print(
            f"NOVO SCAFFOLDS ANALYSIS FOR {scaffolds_type} DONE in {end - start:.04} seconds"
        )
    ####################################
