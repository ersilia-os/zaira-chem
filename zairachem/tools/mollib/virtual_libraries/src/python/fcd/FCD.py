...
def get_predictions(gen_mol):
    ...
    model = load_model(os.path.join(path_here,'model_FCD_all.h5'), 
    ...
    return model.predict_generator(predict_my_generator(gen_mol, batch_size=128), steps=np.ceil(len(gen_mol) / 128))
...
if __name__ == "__main__":
    ...
    mols = pd.read_csv("generated_smiles/endogena.smi", header=None)[0].values[:5000]  # take at least 5000 molecules
    ...
    FCD = calculate_frechet_distance(mu1=np.mean(gen_mol_act, axis=0), mu2=d['mu_chembl'], sigma1=np.cov(gen_mol_act.T),
                                     sigma2=d['cov_chembl'])
    print("FCD: %.4f" % FCD)
