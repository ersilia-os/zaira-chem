...
mols1 = pd.read_csv(sys.argv[1], header=None)[0].values[
    :5000
]  # take at least 5000 molecules
mols2 = pd.read_csv(sys.argv[2], header=None)[0].values[
    :5000
]  # take at least 5000 molecules
...
FCD = calculate_frechet_distance(
    mu1=np.mean(mol_act1, axis=0),
    mu2=np.mean(mol_act2, axis=0),
    sigma1=np.cov(mol_act1.T),
    sigma2=np.cov(mol_act2.T),
)
print("FCD: %.4f" % FCD)
