...
ESTIMATORS = ["rf", "extra_tree", "xgboost", "lgbm"]  # None
...
class Fitter(BaseEstimatorIndividual):
    ...
    def _get_X_y(self, task, is_clf, force_fit=False):
        self.logger.debug("Selecting features")
        X = self._get_X()
        y = self._get_y(task)
        filename_reducer = os.path.join(
            self.trained_path, self.model_id, task + "_reducer.joblib"
        )
        if not os.path.exists(filename_reducer) or force_fit:
            if is_clf:
                max_n = np.min([X.shape[0], X.shape[1], 500])
                reducer = SelectKBest(f_classif, k=max_n)
                X = reducer.fit_transform(X, y)
            else:
                max_n = np.min([X.shape[0], X.shape[1], 500])
                reducer = SelectKBest(f_regression, k=max_n)
                X = reducer.fit_transform(X, y)
            joblib.dump(reducer, filename_reducer)
        else:
            reducer = joblib.load(filename_reducer)
        self.logger.debug("Done with feature selection")
        return X, y
    ...
    def run_heavy(self, time_budget_sec=None):
        ...
        for t in self._get_reg_tasks():
            X, y = self._get_X_y(t, is_clf=False, force_fit=True)
        ...
        for t in self._get_clf_tasks():
            X, y = self._get_X_y(t, is_clf=True, force_fit=True)
    ...
    def run_simple(self, time_budget_sec):
        ...
        for t in self._get_reg_tasks():
            X, y = self._get_X_y(t, is_clf=False, force_fit=False)
            model = FlamlRegressor()
            model.fit_simple(
                X[train_idxs],
                y[train_idxs],
        ...
        for t in self._get_clf_tasks():
            X, y = self._get_X_y(t, is_clf=True, force_fit=False)
            model = FlamlClassifier()
            model.fit_simple(
                X[train_idxs],
                y[train_idxs],
